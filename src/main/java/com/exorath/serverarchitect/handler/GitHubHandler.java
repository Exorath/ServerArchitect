...
public class GitHubHandler implements ConfigHandler {
    @Override
    public void loadPlugins(final Map<String, Object> configSection, File pluginsDir) {
        configSection.entrySet().parallelStream()
                .filter(pluginMapById -> pluginMapById.getValue() instanceof Map)
                .forEach(pluginMapById -> loadPlugin((Map) pluginMapById.getValue(), pluginsDir, 2));
//        for (Map.Entry<String, Object> pluginMapById : configSection.entrySet()) {
//            if (pluginMapById.getValue() instanceof Map) {
//                loadPlugin((Map) pluginMapById.getValue(), pluginsDir, 2);
//            }
//        }
    }

    private void loadPlugin(Map<String, Object> configSection, File pluginsDir, int trialsRemaining) {
        trialsRemaining--;
        try {
            ...
            if (trialsRemaining < 0)
                ...
                loadPlugin(configSection, pluginsDir, trialsRemaining);
        }
    }
